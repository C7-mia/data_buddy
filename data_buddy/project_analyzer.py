...
def safe_load_and_clean(file_name: str):
    ...
    try:
        if ext == ".csv":
            df = retry_with_exponential_backoff(lambda: pd.read_csv(file_path))
            if df.empty:
        ...
        if ext == ".ipynb":
            return None, extract_csv_from_ipynb(file_path)
...
def unpack_and_list_data(zip_name: str, config: BuddyConfig | None = None):
    config = config or BuddyConfig()
    try:
        with zipfile.ZipFile(zip_name, "r") as zip_ref:
            folder_path = config.extracted_data_dir
            folder_path.mkdir(parents=True, exist_ok=True)
            zip_ref.extractall(folder_path)
            all_files = [entry.name for entry in folder_path.iterdir()]
            return str(folder_path), all_files
    except Exception as e:
        return None, f"Unzip Error: {str(e)}"


def extract_csv_from_ipynb(ipynb_file: str | Path) -> str:
    try:
        with Path(ipynb_file).open("r", encoding="utf-8") as f:
            nb = json.load(f)
...
def create_visual_report(
    ...
    config = config or BuddyConfig()
    resolved_output = Path(output_path) if output_path else config.default_chart_output
...
def generate_notebook_code(
    ...
    config = config or BuddyConfig()
    resolved_output = (
        Path(output_path) if output_path else config.default_generated_script
    )
    resolved_input = Path(file_name)
    code = f"""\
...
df = pd.read_csv(Path("{resolved_input.as_posix()}"))
print("📊 Data Summary:")
